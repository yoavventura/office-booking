...
export default function Admin() {
  ...
  return (
    <div style={{ display: 'flex', gap: '24px', alignItems: 'flex-start' }}>
      ...
      {/* Sidebar directory */}
      <div className="card" style={{ width: '160px', flexShrink: 0, padding: '8px', position: 'sticky', top: '24px' }}>
        <div style={{ fontSize: '11px', fontWeight: '600', color: 'var(--gray-400)', textTransform: 'uppercase', letterSpacing: '0.6px', padding: '4px 8px 8px' }}>
          Sections
        </div>
        {SECTIONS.map(s => (
          <button
            ...
            style={{
              ...
              border: 'none', textAlign: 'left', fontSize: '13px', fontWeight: '500',
              background: active === s.key ? 'var(--primary-light)' : 'transparent',
              color: active === s.key ? 'var(--primary)' : 'var(--gray-600)',
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '40px', minWidth: 0 }}>
        ...
        <section ref={refs.rooms} data-section="rooms">
        ...
        <section ref={refs.users} data-section="users">
          <div style={{ fontSize: '16px', fontWeight: '700', color: 'var(--gray-800)', marginBottom: '16px', paddingBottom: '10px', borderBottom: '2px solid var(--gray-100)' }}>
            👥 Users
          </div>
          <UsersSection />
...
function RoomsSection() {
  ...
  function loadRooms() { api.get('/rooms').then(res => setRooms(res.data)).catch(console.error); }
  useEffect(() => { loadRooms(); }, []);

  function openAdd() { setForm(emptyForm); setEditRoom(null); setShowForm(true); setError(''); }
  function openEdit(room) { setForm({ ...room, amenities: (room.amenities || []).join(', ') }); setEditRoom(room); setShowForm(true); setError(''); }
    ...
    <div>
              ...
              <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                {ROOM_COLORS.map(c => (
                  <button key={c} type="button" onClick={() => setForm(p => ({ ...p, color: c }))}
                    style={{ width: '26px', height: '26px', borderRadius: '50%', background: c, border: form.color === c ? '3px solid var(--gray-800)' : '2px solid white', outline: form.color === c ? '2px solid var(--gray-400)' : 'none', cursor: 'pointer' }} />
                ))}
              </div>
            </div>
      ...
      <div className="card">
        <table className="table">
          <thead><tr><th>Room</th><th>Type</th><th>Capacity</th><th>Floor</th><th>Amenities</th><th></th></tr></thead>
          <tbody>
            {rooms.map(room => (
              <tr key={room.id}>
                <td><div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}><span style={{ width: '12px', height: '12px', borderRadius: '3px', background: room.color, flexShrink: 0 }} /><span style={{ fontWeight: '500' }}>{room.name}</span></div></td>
            ...
            {rooms.length === 0 && <tr><td colSpan={6} style={{ textAlign: 'center', color: 'var(--gray-400)', padding: '32px' }}>No rooms yet</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
...
function UsersSection() {
  ...
  function loadUsers() { api.get('/users').then(res => setUsers(res.data)).catch(console.error); }
  useEffect(() => { loadUsers(); }, []);

  function openAdd() { setForm(emptyForm); setEditUser(null); setShowForm(true); setError(''); }
  function openEdit(user) { setForm({ name: user.name, email: user.email, password: '', role: user.role }); setEditUser(user); setShowForm(true); setError(''); }
          ...
          <tbody>
            {users.map(user => (
              <tr key={user.id} style={{ opacity: user.is_active ? 1 : 0.4 }}>
                ...
                <td><span className={`badge ${user.is_active ? 'badge-confirmed' : 'badge-cancelled'}`}>{user.is_active ? 'Active' : 'Inactive'}</span></td>
                <td><div style={{ display: 'flex', gap: '6px' }}>
                  <button className="btn btn-ghost btn-sm" onClick={() => openEdit(user)}>Edit</button>
                  {user.is_active && <button className="btn btn-ghost btn-sm" style={{ color: 'var(--danger)' }} onClick={() => handleDeactivate(user)}>Deactivate</button>}
                </div></td>
              </tr>
            ))}
            {users.length === 0 && <tr><td colSpan={5} style={{ textAlign: 'center', color: 'var(--gray-400)', padding: '32px' }}>No users</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
...
function OutlookSection() {
  ...
  async function handleTest() {
    setTestResult(''); setError(''); setTesting(true);
    try {
      const res = await api.post('/settings/outlook/test', form);
      setTestResult(res.data.message);
    } catch (err) { setTestResult('✗ ' + (err.response?.data?.error || 'Connection failed')); }
        ...
        <form onSubmit={handleSave} style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
          ...
          <div className="form-group">
            ...
            <input type="password" className="form-input"
              ...
              value={form.clientSecret} onChange={e => setForm(p => ({ ...p, clientSecret: e.target.value }))} />
          </div>

          {testResult && (
            <div className={`alert ${testResult.startsWith('✗') ? 'alert-danger' : 'alert-success'}`}>{testResult}</div>
          )}
